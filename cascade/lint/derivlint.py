...
def _param_types(
    ...
    types: dict[str, str | None] = {
        arg.arg: _annotation_name(arg.annotation) for arg in every
    }
...
def _collect(
    source: str, model_names: set[str], path: str
) -> tuple[list[_Collected], list[Violation]]:
...
def check_derivations(
    source: str, model_names: set[str], path: str = "<string>"
) -> list[Violation]:
...
def main(argv: list[str]) -> int:
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        prog="python -m cascade.lint.derivlint",
    ...
    model_names: set[str] = {
        name.strip() for name in namespace.models.split(",") if name.strip()
    }
