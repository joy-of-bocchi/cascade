...
visible region each frame — flat cost at any zoom depth or canvas size.
    ...
    python -m cascade.render.wrapmmd diagram.mmd   ->   diagram.view.html
"""
...
def wrap(name: str, mermaid_src: str) -> str:
    return TEMPLATE.replace("__NAME__", html.escape(name)).replace(
        "__MMD__", mermaid_src
    )
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: python -m cascade.render.wrapmmd <diagram.mmd>")
    mmd_path = Path(sys.argv[1])
    out_path = mmd_path.with_suffix(".view.html")
    mermaid_src = mmd_path.read_text()
    page = wrap(mmd_path.stem, mermaid_src)
