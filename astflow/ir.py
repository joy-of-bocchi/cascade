...
class EdgeKind(StrEnum):
    ...
    DERIVED_AFTER = "derived_after"  # a field populated after construction (post-ctor mutation)
...
class ArgResolution(BaseModel):
    ...
    resolves_to: str | None = None  # entity name (bare or wrapped in list/dict/… ), or None if unresolved
...
class MutationFact(BaseModel):
    ...
    targets: list[str] = Field(default_factory=list)  # attribute paths written, e.g. "summary.total"
...
class DecisionFact(BaseModel):
    ...
    site: Site
    gates: list[str] = Field(default_factory=list)  # entity names whose construction is inside this branch
...
class FactIR(BaseModel):
    ...
    unresolved: list[str] = Field(default_factory=list)  # notes where type resolution went dark (untyped seams)
