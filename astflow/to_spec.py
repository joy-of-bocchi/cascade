...
from ir import FactIR  # noqa: E402
...
if __name__ == "__main__":
    ...
    import fixtures.sample as sample
    from ir import (
        ArgResolution,
        ConstructionFact,
        DecisionFact,
    ...
    violations = verify(spec, ir, entity_classes)
    print("=== verify() ===")
    print(violations)
    assert violations == [], f"expected clean verify, got: {violations}"

    edge_set = {(e.src, e.dst) for e in spec.edges}
    assert ("leaf", "mid") in edge_set, "missing build-in leaf -> mid"
    assert ("mid", "whole") in edge_set, "missing build-in mid -> whole"
    assert ("whole", "summary") in edge_set, "missing derived whole -> summary"
    assert ("summary", CONSUMERS_ID) in edge_set, "missing read summary -> consumers"
    assert any(e.dst == f"dormant{VOID_SUFFIX}" for e in spec.edges), (
        "missing dormant marker"
    )
    assert any(e.src == "dec_0" and e.dst in ("leaf", "mid") for e in spec.edges), (
