...
def base(name: str, type_repr: str, description: str = "") -> CanonicalField:
    return CanonicalField(name=name, type_repr=type_repr, origin=Origin.BASE, description=description)


def derived(name: str, type_repr: str, from_: list[str], description: str = "") -> CanonicalField:
...
def _derivation_cycles(canonical: dict[str, CanonicalField]) -> list[list[str]]:
    adjacency: dict[str, list[str]] = {name: list(cf.derived_from) for name, cf in canonical.items()}
    color: dict[str, int] = {name: 0 for name in adjacency}  # 0 unseen, 1 active, 2 done
