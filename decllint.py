...
def check_single_declaration(models: list[type[BaseModel]]) -> list[DeclViolation]:
    ...
    for model in models:
        names = set(model.model_fields) | set(getattr(model, "model_computed_fields", {}) or {})
        for name in names:
...
def report(title: str, models: list[type[BaseModel]]) -> list[DeclViolation]:
    ...
    for violation in violations:
        print(f"  [duplicate_declaration] '{violation.name}' declared on {', '.join(violation.declarers)}")
    return violations
